...
#include "../headers/screen.h" /* for freeResources */
#include <stdbool.h>
...
const char *menu_launcher_names[]={
	"GMenu2X",
	"RetroFE",
	"SimpleMenu",
	"AdvMenu"};
...
typedef enum {
	ID_LAUNCHER_GMENU2X=0,
	ID_LAUNCHER_RETROFE,
	ID_LAUNCHER_SIMPLEMENU,
	ID_LAUNCHER_ADVMENU,
	NUM_OF_LAUNCHER} tenum_launcher;

static int index_of_launcher=0;
#endif

/// -------------- FUNCTIONS IMPLEMENTATION --------------

bool executeRawPath(const char *shellCmd)
{
    bool retVal = false;
	char executionString[50]="exec ";
    //MENU_DEBUG_PRINTF( "Attempting to launch: %s\n", shellCmd);
    //std::string executionString = "exec " + std::string(shellCmd);
	strcat(executionString, shellCmd);
	//sprintf(executionString, "sh -c %s", shellCmd);
    //MENU_DEBUG_PRINTF("Running: %s\n", executionString.c_str());

    if(system(executionString) != 0) {
...
static void add_menu_zone(ENUM_MENU_TYPE menu_type)
{
	...
	case MENU_TYPE_VOLUME:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_VOLUME), text_color);
	...
	case MENU_TYPE_BRIGHTNESS:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_BRIGHTNESS), text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2 - padding_y_from_center_menu_zone;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
	...
	case MENU_TYPE_THEME:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_SET_THEME), text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2 - padding_y_from_center_menu_zone*2;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
		break;
	...
	case MENU_TYPE_LAUNCHER:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_SET_LAUNCHER), text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2 - padding_y_from_center_menu_zone*2;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
/*
		...
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, menu_launcher_names[index_of_launcher], text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
...
*/
	...
	case MENU_TYPE_RO_RW:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_SET_SYSTEM_), text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2 - padding_y_from_center_menu_zone*2;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
	    break;
	...
	case MENU_TYPE_ADVANCED:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, "ADVANCED", text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
		break;
	...
	case MENU_TYPE_EXIT:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_EXIT), text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
		break;
	...
	case MENU_TYPE_POWERDOWN:
		...
		/// ------ Text ------
		text_surface = TTF_RenderUTF8_Blended(menu_title_font, ui_getstring(UI_POWERDOWN), text_color);
		text_pos.x = (surface->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		text_pos.y = surface->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2;
		SDL_BlitSurface(text_surface, NULL, surface, &text_pos);
		break;
...
static void menu_screen_refresh(SDL_Surface *screen, int menuItem, int prevItem, int scroll, uint8_t menu_confirmation, uint8_t menu_action)
{
	...
	/// --------- No Scroll ? Blitting menu-specific info
		...
		case MENU_TYPE_SAVE:
			...
			if(menu_action){
				sprintf(text_tmp, ui_getstring(UI_Saving___));
				text_surface = TTF_RenderUTF8_Blended(menu_info_font, text_tmp, text_color);
			}
			else{
				if(menu_confirmation){
					sprintf(text_tmp, ui_getstring(UI_Are_you_sure_));
					text_surface = TTF_RenderUTF8_Blended(menu_info_font, text_tmp, text_color);
				}
				else{
					if (state_exists(state_slot)) {
						text_surface = TTF_RenderUTF8_Blended(menu_info_font, "Used", text_color);
					} else {
						text_surface = TTF_RenderUTF8_Blended(menu_info_font, "Free", text_color);
					}
				}
			}
			text_pos.x = (screen->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
		...
		case MENU_TYPE_LOAD:
			...
			if(menu_action){
				sprintf(text_tmp, ui_getstring(UI_Loading___));
				text_surface = TTF_RenderUTF8_Blended(menu_info_font, text_tmp, text_color);
			}
			else{
				if(menu_confirmation){
		...
		case MENU_TYPE_USB:
			/// ---- Write slot -----
			sprintf(text_tmp, "%s", usb_sharing ? ui_getstring(UI_EJECT_USB):ui_getstring(UI_MOUNT_USB));
			...
			if(menu_action){
				sprintf(text_tmp, ui_getstring(UI_in_progress____));
				text_surface = TTF_RenderUTF8_Blended(menu_info_font, text_tmp, text_color);
				text_pos.x = (screen->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
				text_pos.y = screen->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2 + 2*padding_y_from_center_menu_zone;
				SDL_BlitSurface(text_surface, NULL, screen, &text_pos);
		...
		case MENU_TYPE_POWERDOWN:
			...
			if(menu_action){
				sprintf(text_tmp, ui_getstring(UI_Shutting_down___));
				text_surface = TTF_RenderUTF8_Blended(menu_info_font, text_tmp, text_color);
				text_pos.x = (screen->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
				text_pos.y = screen->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2 + 2*padding_y_from_center_menu_zone;
				SDL_BlitSurface(text_surface, NULL, screen, &text_pos);
...
#ifdef HAS_MENU_RO_RW
		case MENU_TYPE_RO_RW:
			sprintf(text_tmp, "%s", read_write ? ui_getstring(UI_READ_ONLY):ui_getstring(UI_READ_WRITE));
			text_surface = TTF_RenderUTF8_Blended(menu_title_font, text_tmp, text_color);
			text_pos.x = (screen->w - MENU_ZONE_WIDTH)/2 + (MENU_ZONE_WIDTH - text_surface->w)/2;
			text_pos.y = screen->h - MENU_ZONE_HEIGHT/2 - text_surface->h/2;
			SDL_BlitSurface(text_surface, NULL, screen, &text_pos);
				...
				case SDL_KEYDOWN:
					...
					case SDLK_LEFT:
						...
						if(idx_menus[menuItem] == MENU_TYPE_LAUNCHER){
							index_of_launcher = (NUM_OF_LAUNCHER + index_of_launcher - 1) % NUM_OF_LAUNCHER;
							/// ------ Refresh screen ------
							screen_refresh = 1;
						} else
#endif
...
#ifdef HAS_MENU_LAUNCHER
						if(idx_menus[menuItem] == MENU_TYPE_LAUNCHER){
							index_of_launcher = (index_of_launcher + 1) % NUM_OF_LAUNCHER;
							/// ------ Refresh screen ------
							screen_refresh = 1;
						} else
#endif
						...
						if(idx_menus[menuItem] == MENU_TYPE_SAVE){
						  ...
						  if(idx_menus[menuItem] == MENU_TYPE_LAUNCHER){
							if(menu_confirmation){
								...
								/// ----- Shell cmd ----
								if (index_of_launcher == ID_LAUNCHER_GMENU2X) {
									MENU_DEBUG_PRINTF("Running command: %s\n", SHELL_CMD_SET_LAUNCHER_GMENU2X);
									executeRawPath(SHELL_CMD_SET_LAUNCHER_GMENU2X);
								}
								else if (index_of_launcher == ID_LAUNCHER_RETROFE) {
									executeRawPath(SHELL_CMD_SET_LAUNCHER_RETROFE);
								}
								else if (index_of_launcher == ID_LAUNCHER_SIMPLEMENU) {
									executeRawPath(SHELL_CMD_SET_LAUNCHER_SIMPLEMENU);
								}
								else if (index_of_launcher == ID_LAUNCHER_ADVMENU) {
									executeRawPath(SHELL_CMD_SET_LAUNCHER_ADVMENU);
								}
